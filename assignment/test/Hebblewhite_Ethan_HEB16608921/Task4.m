...
%origional data from cartesian to polar data. but before this happens all
%the numbers in column 1 of Boundary_Star_Fish will have the mean of the
%column taken away from evey number in the coloumn then it will have the 
%turned from cartesian to polar data, which can now be used for our signatures.
%Theta_StarFish represents an angle
%Radius_starfish represents the radius. 
%we minus the mean to 

%works out the mean for each column
mean_StarFish_1 = mean(Boundary_Star_Fish(: , 1));
mean_StarFish_2 = mean(Boundary_Star_Fish(: , 2));

[Theta_StarFish, Radius_StarFish] = cart2pol(Boundary_Star_Fish(:,2) - mean_StarFish_2,Boundary_Star_Fish(:,1) - mean_StarFish_1);

%other Objects that i have selected from image 2 in the image boundary and signature creation
B = bwboundaries(Binary_Image_2);
...
%means for each column for each object 
%object A
mean_Object_A_1 = mean(Boundary_Object_A(:,1)); 
mean_Object_A_2 = mean(Boundary_Object_A(:,2)); 

%object B
mean_Object_B_1 = mean(Boundary_Object_B(:,1)); 
mean_Object_B_2 = mean(Boundary_Object_B(:,2)); 

%object C
mean_Object_C_1 = mean(Boundary_Object_C(:,1)); 
mean_Object_C_2 = mean(Boundary_Object_C(:,2)); 

%object D
mean_Object_D_1 = mean(Boundary_Object_D(:,1)); 
mean_Object_D_2 = mean(Boundary_Object_D(:,2)); 

%signature creation for the objects
[Theta_Object_A, Radius_Object_A] = cart2pol(Boundary_Object_A(:,2) - mean_Object_A_2,Boundary_Object_A(:,1) - mean_Object_A_1);
[Theta_Object_B, Radius_Object_B] = cart2pol(Boundary_Object_B(:,2) - mean_Object_B_2,Boundary_Object_B(:,1) - mean_Object_B_1);
[Theta_Object_C, Radius_Object_C] = cart2pol(Boundary_Object_C(:,2) - mean_Object_C_2,Boundary_Object_C(:,1) - mean_Object_C_1);
[Theta_Object_D, Radius_Object_D] = cart2pol(Boundary_Object_D(:,2) - mean_Object_D_2,Boundary_Object_D(:,1) - mean_Object_D_1);
